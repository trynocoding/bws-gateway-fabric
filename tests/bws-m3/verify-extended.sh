...
readonly http_port="${BWS_M3_EXTENDED_HTTP_PORT:-28080}"
readonly https_port="${BWS_M3_EXTENDED_HTTPS_PORT:-28443}"
readonly metrics_port="${BWS_M3_EXTENDED_METRICS_PORT:-29113}"
...
access_log_contains_probe() {
    local output
    output="$(kubectl -n "${namespace}" logs -l "${selector}" -c nginx \
        --since-time="${probe_start}" --prefix=true 2>/dev/null)"
    grep -q "${probe_path}" <<<"${output}"
}

access_log_configured() {
    kubectl -n "${namespace}" exec -c nginx "${pod}" -- \
        grep -q 'log_format bws_m3' \
        /etc/nginx/includes/SnippetsPolicy_http_bws-m3-bws-m3-access-log.conf
}
...
if [[ -z "${dataplane_deployment}" || -z "${service}" || -z "${pod}" ]]; then
    echo "could not resolve the M3 data-plane resources" >&2
    exit 1
fi
...
kubectl -n "${namespace}" exec -c nginx "${pod}" -- bash -c '
    grep -q "js_import modules/njs/httpmatches.js" /etc/nginx/nginx.conf
    grep -q "map \$http_upgrade \$connection_upgrade" /etc/nginx/conf.d/http.conf
    grep -q "proxy_set_header Upgrade" /etc/nginx/conf.d/http.conf
    /opt/bws/bin/bws.sh -p /opt/bws -c /etc/nginx/nginx.conf -t
' >/dev/null
...
kubectl -n "${namespace}" rollout status deployment/bws-m3-websocket --timeout=3m
kubectl -n "${namespace}" rollout status "deployment/${dataplane_deployment}" --timeout=5m
...
probe_path="/bws-m3-log-probe-$(date +%s)"
...
error_logs="$(kubectl -n "${namespace}" logs "${pod}" -c nginx)"
...
echo "M3 extended verification passed"
...
echo "not validated by this script: gRPC, HTTP/3, stream, HPA, license rotation, or longevity"
