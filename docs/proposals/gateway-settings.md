...
This Enhancement Proposal introduces the `BwsProxy` API, which allows Cluster Operators to define configuration at the GatewayClass level for all Gateways (proxies) under that Class. This configuration is attached via the GatewayClass `parametersRef` field.

## Goals

- Define the Gateway settings.
- Define an API for the BwsProxy CRD.

## Non-Goals

- Provide implementation details for implementing the BwsProxy configuration.
...
These settings apply to the `main`, `http`, and/or `stream` contexts of the NGINX config. The `BwsProxy` CRD will contain these settings.

To begin, the `BwsProxy` CRD will include the following NGINX directives (focusing on OpenTelemetry tracing):
...
The `BwsProxy` API is a CRD that is a part of the `gateway.bessystem.com` Group. It will be referenced in the `parametersRef` field of a GatewayClass. It will live at the cluster scope.
...
For example, an `BwsProxy` named `proxy-settings` would be referenced as follows:
...
metadata:
    name: nginx
spec:
    controllerName: gateway.bessystem.com/nginx-gateway-controller
    parametersRef:
...
Below is the Golang API for the `BwsProxy` API:
...
There are two Conditions on the GatewayClass status to consider when using the `parametersRef`. The first is a `ResolvedRefs` Condition. If the `BwsProxy` reference cannot be found, then this Condition is set to `False`.

NGINX Gateway Fabric must set this Condition on the GatewayClass affected by an `BwsProxy`.
...
- This Condition should be added when the affected object starts being affected by an `BwsProxy`.
- When the `BwsProxy` affecting that object is removed, the Condition should be removed.
- The Observed Generation is the generation of the GatewayClass, not the generation of the `BwsProxy`.

The other condition is the existing `Accepted` condition on the GatewayClass. There is an existing reason for this Condition, `InvalidParameters`, that sets `Accepted` to `False` when the `parametersRef` fields are invalid. However, this could lead to downstream problems. For example, if a GatewayClass is `Accepted`, and then `BwsProxy` are updated to something invalid, then marking the GatewayClass as `not Accepted` would result in the entire downstream configuration tree being nullified. This is a large disruption.

The proposition is to instead keep the GatewayClass as `Accepted` even if the `BwsProxy` are invalid, but still set the reason to `InvalidParameters` and include a detailed message about the issue. In this case, default values will be reverted to for the settings, and downstream configuration will remain intact. There may be impact by reverting to defaults, but this impact is likely less than the impact of completely nullifying all Gateways/Routes/etc. that live under the GatewayClass if we instead marked it as `not Accepted`.
...
Validating all fields in the `BwsProxy` is critical to ensuring that the NGINX config generated by NGINX Gateway Fabric is correct and secure.

All fields in the `BwsProxy` will be validated with Open API Schema. If the Open API Schema validation rules are not sufficient, we will use [CEL](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#validation-rules).
