...
This Enhancement Proposal extends the [BwsProxy API](gateway-settings.md), to allow users to configure a method to rewrite the client's IP address to the original client's IP when NGF is fronted by another load balancer or proxy.
...
This API will be added to the `BwsProxy` CRD that is a part of the `gateway.bessystem.com` Group. It will be referenced in the `parametersRef` field of a GatewayClass. It will live at the cluster scope.
...
Below is the Golang API for the `RewriteClientIP` field on `BwsProxy`. Note, all other `BwsProxy` fields have been omitted to keep the focus on `RewriteClientIP`.
...
Status is set on the GatewayClass, not the `BwsProxy` resource. If the `BwsProxy` is invalid, set the `Accepted` condition reason on the GatewayClass to `InvalidParameters` but still mark `Accepted` as `True`. See [gateway settings proposal](gateway-settings.md#status) for more details on status.
...
- [Gateway Settings (BwsProxy CRD)](gateway-settings.md)
